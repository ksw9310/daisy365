...
export function SettingEditModal({ editKey, settings, onClose, onSave }) {
  const titleMap = {
    cafeName: '카페 닉네임',
    stampsRequired: '쿠폰 도장 수',
    reward: '쿠폰 보상',
    password: '비밀번호 변경',
    registerUrl: '등록 페이지 URL',
  };
  const labelMap = {
    cafeName: '카페 닉네임',
    stampsRequired: '도장 수 (1~20)',
    reward: '쿠폰 보상 내용',
    password: '새 비밀번호 (4자리)',
    registerUrl: 'Vercel/Netlify 배포 후 받은 URL',
  };

  const [v1, setV1] = useState(
    editKey === 'password' ? '' : String(settings[editKey] || ''),
  );
        ...
        <div className="input-group">
          ...
          <input
            type={
              editKey === 'password'
                ? 'password'
                : editKey === 'stampsRequired'
                  ? 'number'
                  : 'text'
            }
            inputMode={
              editKey === 'password' || editKey === 'stampsRequired'
                ? 'numeric'
                : 'text'
            }
