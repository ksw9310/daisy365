  ...
  return (
    <div className="screen active">
      ...
      <div className="settings-block">
        {[
          { key: 'cafeName',       icon: '☕', title: '카페 이름',          sub: D.settings.cafeName },
          ...
          <button key={key} className="settings-row" onClick={() => onEdit(key)}>
            ...
            <div className="s-text"><h3>{title}</h3><p>{sub}</p></div>
      ...
      {qrUrl && (
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#6B4226', marginBottom: 12 }}>📷 손님용 QR 코드</div>
          <div ref={qrRef} style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }} />
          <div style={{ fontSize: 11, color: '#999', lineHeight: 1.6 }}>
            카운터에 출력하거나 화면을 보여주세요<br />손님이 스캔하면 등록 페이지가 열려요
          </div>
        </div>
      )}
      ...
      <div className="settings-block">
        <button className="settings-row" onClick={onExport}>
          <span className="s-icon">📤</span>
          <div className="s-text"><h3>손님 목록 내보내기</h3><p>CSV 파일로 다운로드</p></div>
          <span className="s-arrow">›</span>
        </button>
        <button className="settings-row" onClick={onReset}>
          <span className="s-icon">🗑️</span>
          <div className="s-text"><h3>전체 데이터 초기화</h3><p style={{ color: '#E53935' }}>모든 손님 정보 삭제</p></div>
          <span className="s-arrow">›</span>
        </button>
      </div>

      <div style={{ textAlign: 'center', padding: '24px 20px 10px', color: '#CCC', fontSize: 12 }}>
