...
export default function HomeTab({ D, onStamp, onNewCustomer }) {
  ...
  const todayCount = D.activity.filter(
    (a) => a.date === today && a.type === 'stamp',
  ).length;
  ...
  return (
    <div className="screen active">
      {/* ── 헤더 ── */}
      <div className="daisy-banner">
        <div className="daisy-logo-row">
          <div className="daisy-logo-box">🌼</div>
          <div>
            <div className="daisy-cafe-name">{D.settings.cafeName}</div>
            <div className="daisy-date">{dateStr}</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="input-group" style={{ marginBottom: 10 }}>
          <label>손님 닉네임 또는 전화번호 뒷 4자리</label>
          <input
            id="quickInput"
            type="text"
        ...
        <button className="btn btn-primary" onClick={tryStamp}>
          🌼 도장 찍기
        </button>
